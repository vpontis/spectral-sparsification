...
\abstract{
A spectral sparsification algorithm takes an input graph and returns a new graph with fewer edges that preserves up to an approximation a term called the Laplacian of the graph. The Laplacian of the graph relates to the connectivity of the graph and is a strong property; graphs that have similar Laplacians also have similar cut values as well as similar connectivities and other important properties. 
  
In this paper we will give a introduction to spectral graph theory and sparsification to a reader who has the background of an undergraduate degree in computer science. We will describe two sampling algorithms: one that considers vertex degree and one that considers effective resistance, giving intuition behind both algorithms. We also will give the necessary background work, including material in linear algebra and effective resistance, needed for the analysis of the algorithms presented. 
}
...
%For the rest of the paper, we will only be considering the sparsification of unweighted graphs. Sparisification of weighted graphs is a bit more involved but heavily draws on the concepts and algorithms used in unweighted graphs.\footnote{We also only consider undirected graphs. Directed graphs are much more complicated and there is not much progress on sparsifying directed graphs.}
...
% The basic idea of the algorithm is to independently select edge $e$ with probability $p_e$ proportional to the degree of the vertex. If an edge is selected, it is added into $\tilde{G}$ and given a weight of $1/p_e$. The main result is that for an accurate selection of $p_e$, the graph $\tilde{G}$ will be a good spectral sparisfier of $G$ with high probability.
